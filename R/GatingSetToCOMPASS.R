...
##' @export
COMPASSContainerFromGatingSet <- function(gs=NULL,node=NULL,filter.fun=NULL,individual_id="PTID",sample_id="name",stimulation_id="Stim",mp=NULL,countFilterThreshold=5000,matchmethod=c("regex","Levenshtein")){
  if(require(flowWorkspace)){
    ...
    if(is.null(mp)){
      ...
      matchmethod<-match.arg(arg=matchmethod,choices=c("regex","Levenshtein"))
      if(matchmethod=="Levenshtein"){
        distances<-adist(child.nodes[,child.nodes.upper],na.omit(params[,desc.upper]))
        matching<-as.vector(solve_LSAP(distances))
        matched<-cbind(na.omit(params)[matching,list(name,desc)],node=child.nodes[,child.nodes])
        map<-matched
      }else{
