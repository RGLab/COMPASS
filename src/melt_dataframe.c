...
#define HANDLE_CASE(RTYPE, CTYPE)                                             \
  case RTYPE: {                                                               \
    ...
    for (int i = 0; i < times; ++i) {                                         \
      memcpy((char*)DATAPTR(out) + (i* len* sz), (char*)DATAPTR(x), len* sz); \
    }                                                                         \
    UNPROTECT(1);                                                             \
    return out;                                                               \
  }
...
SEXP melt_dataframe(SEXP x,
  ...
  if (mt > STRSXP) {
    error("Error: cannot melt data.frames w/ elements of type '%s'",
          CHAR(type2str(mt)));
  }
...
#define HANDLE_CASE(RTYPE, CTYPE)                                    \
  case RTYPE: {                                                      \
    ...
    for (int i = 0; i < nColRep; ++i) {                              \
      ...
      memcpy((char*)DATAPTR(value_SEXP) + (i* nRow * sizeof(CTYPE)), \
             (char*)DATAPTR(tmp),                                    \
             nRow * sizeof(CTYPE));                                  \
      if (TYPEOF(VECTOR_ELT(x, val_ind[i])) != mt) {                 \
        UNPROTECT(1);                                                \
...
#undef HANDLE_CASE
  ...
  for (int i = 0; i < nColStack; ++i) {
    ...
    if (isFactor(VECTOR_ELT(x, id_ind[i]))) {
      ...
      setAttrib(VECTOR_ELT(out, i),
                R_LevelsSymbol,
                getAttrib(VECTOR_ELT(x, id_ind[i]), R_LevelsSymbol));
    }
  }
  ...
  SET_VECTOR_ELT(out,
                 nColStack,
                 rep_each_char(getAttrib(x, R_NamesSymbol), val_ind_, nRow));
